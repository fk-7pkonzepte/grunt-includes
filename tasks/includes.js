...
module.exports = function (grunt) {
    ...
    function commentStyle(p) {
        var comments, ext = path.extname(p).slice(1);
        comments = {
            js: "/* %s */",
            css: "/* %s */",
            html: "<!-- %s -->"
    ...
    function recurse(p, opts, level, included, indents) {
        ...
        if (opts.wrapper !== '' && level === 1) {
            ...
            if (grunt.file.isFile(opts.wrapper)) {
                opts.wrapper = grunt.file.read(opts.wrapper);
            }

            if (opts.wrapper.match(opts.wrapperFileRegexp)) {
                var currentWrapper = opts.wrapper.split(newlineStyle(opts.wrapper)).map(function (line) {
                    ...
                    if (line.match(opts.wrapperFileRegexp)) {
                        ...
                        src = src.split(newline).map(function (srcline, i) {
        ...
        compiled = src.map(function (line) {
            ...
            if (match) {
                ...
                if (opts.contentModifier) {
                    content = opts.contentModifier(content);
                }
